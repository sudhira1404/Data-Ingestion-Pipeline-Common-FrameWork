...
printf "This next step will cause your web browser to open and launch\ninto a page that asks you to approve this application.\n"
printf "Please approve the application in the browser,\nthen return to this terminal window.\n"
...
REFRESH_TOKEN=$(echo "$OAUTH_RESPONSE" | jq -r '.["refresh_token"]')
...
echo "Scope: ${SCOPE}"
echo "Access Token (Short Term): ${ACCESS_TOKEN}"
echo "Refresh Token (expires in 1 year): ${REFRESH_TOKEN}"
echo 'Use the above code to update the Pinterest secrets'
